...
//      NUMERO MAXIMO DE LINEAS EN MENU SON 16 (PARA MAS HACER SUBMENUS)
	{MENU_ENTRY_CALLBACK,"    = Commodore 16 =    ",0},
	...
	{MENU_ENTRY_CYCLE,(char *)st_memory,MENU_ACTION(2)},
	{MENU_ENTRY_CYCLE,(char *)st_sid,MENU_ACTION(3)},
	{MENU_ENTRY_CYCLE,(char *)st_tapesnd,MENU_ACTION(2)},
	{MENU_ENTRY_CYCLE,(char *)st_tape,MENU_ACTION(2)},
	{MENU_ENTRY_CALLBACK,"Cargar PRG/TAP/D64 \x10",MENU_ACTION(&FileSelector_Show)},
...
int main(int argc,char **argv)
{
	...
	while(1)
	{
		...
		dipsw |= (MENU_CYCLE_VALUE(&topmenu[3])  & 0x3) << 1; //[2:1] 
		dipsw |= (MENU_CYCLE_VALUE(&topmenu[4])  & 0x1) << 3; //[3]
		dipsw |= (MENU_CYCLE_VALUE(&topmenu[5])  & 0x1) << 4; //[4]
		dipsw |= (MENU_CYCLE_VALUE(&topmenu[6])  & 0x3) << 8; //[9:8]
		dipsw |= (MENU_CYCLE_VALUE(&topmenu[7])  & 0x1) << 7; //[7]
